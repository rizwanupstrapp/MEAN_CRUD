...
export class EmailTemplateListComponent implements OnInit {
  ...
  EMAIL_TEMPLATE_CONST: any = [];
  searchObj = { field: "emailTemplate_title", emailTemplate_status: "", search: "" };
  ...
  constructor(
    ...
    titleService.setTitle(
      this.APPNAME + " | " + this.EMAIL_TEMPLATE_CONST.EMAIL_MODULE_NAME
    );
  }
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.EMAIL_TEMPLATE.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.EMAIL_TEMPLATE.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.EMAIL_TEMPLATE.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.EMAIL_TEMPLATE.READ)) {

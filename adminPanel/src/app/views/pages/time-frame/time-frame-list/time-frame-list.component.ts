...
@Component({
  ...
  styleUrls: ["./time-frame-list.component.scss"],
})
export class TimeFrameListComponent implements OnInit {
  ...
  TIMEFRAME_CONST: any = [];
  searchObj = { field: "timeFrame_timeType", timeFrame_status: "", search: "" };
  ...
  constructor(
    ...
    titleService.setTitle(
      this.APPNAME + " | " + this.TIMEFRAME_CONST.MODULE_NAME
    );
  }
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.TIME_FRAME.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TIME_FRAME.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TIME_FRAME.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TIME_FRAME.READ)) {

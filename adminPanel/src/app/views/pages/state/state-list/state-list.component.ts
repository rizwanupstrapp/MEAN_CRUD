...
export class StateListComponent implements OnInit {
    ...
    getPermission() {
        this.localStorageService.getBehaviorView().subscribe((data: any) => {
            if (data && data.length) {
                if (this.checkHasPermission(data, PERMISSIONS.STATE.CREATE)) {
                    this.createPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.STATE.EDIT)) {
                    this.editPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.STATE.DELETE)) {
                    this.deletePermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.STATE.READ)) {

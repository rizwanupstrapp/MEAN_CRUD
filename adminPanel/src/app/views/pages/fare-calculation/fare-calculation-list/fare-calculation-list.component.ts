...
@Component({
  ...
  templateUrl: './fare-calculation-list.component.html',
  styleUrls: ['./fare-calculation-list.component.scss']
})
export class FareCalculationListComponent implements OnInit {
  ...
  searchObj = { field: 'help_title', fareCalculation_status: '', search: '' };
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.FARE_CALCULATION.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.FARE_CALCULATION.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.FARE_CALCULATION.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.FARE_CALCULATION.READ)) {
  ...
  ngAfterViewInit() {
    ...
    this._script.loadScripts('kt-fare-calculation-list', scripts).then(function () {
      ...
      var table = $(document).find('#kt_datatable').DataTable({
        ...
        ajax: {
          url: ApiURIs.listFareCalculation,
          type: 'POST',
          headers: {

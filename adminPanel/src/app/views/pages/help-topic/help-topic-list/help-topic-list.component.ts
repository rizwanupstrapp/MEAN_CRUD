...
@Component({
  ...
  styleUrls: ['./help-topic-list.component.scss']
})
export class HelpTopicListComponent implements OnInit {
  ...
  HELPTOPIC_CONST: any = [];
  searchObj = { field: 'helpTopic_name', helpTopic_status: '', search: '' };
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.HELP_TOPIC.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.HELP_TOPIC.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.HELP_TOPIC.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.HELP_TOPIC.READ)) {

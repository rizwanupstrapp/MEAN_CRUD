...
export class BadgeListComponent implements OnInit {
    ...
    BADGE_CONST: any = [];
    searchObj = { field: 'badge_name', badge_status: '', search: '' };
    ...
    getPermission() {
        this.localStorageService.getBehaviorView().subscribe((data: any) => {
            if (data && data.length) {
                if (this.checkHasPermission(data, PERMISSIONS.BADGE.CREATE)) {
                    this.createPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.BADGE.EDIT)) {
                    this.editPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.BADGE.DELETE)) {
                    this.deletePermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.BADGE.READ)) {

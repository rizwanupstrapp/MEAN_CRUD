...
export class RolesListComponent implements OnInit {
	...
	ROLES_CONST: any = [];
	searchObj = { field: 'role_name', role_status: '', search: '' };
	...
	getPermission() {
		this.localStorageService.getBehaviorView().subscribe((data: any) => {
			if (data && data.length) {
				if (this.checkHasPermission(data, PERMISSIONS.ROLE.CREATE)) {
					this.createPermission = true;
				}
				if (this.checkHasPermission(data, PERMISSIONS.ROLE.EDIT)) {
					this.editPermission = true;
				}
				if (this.checkHasPermission(data, PERMISSIONS.ROLE.DELETE)) {
					this.deletePermission = true;
				}
				if (this.checkHasPermission(data, PERMISSIONS.ROLE.READ)) {

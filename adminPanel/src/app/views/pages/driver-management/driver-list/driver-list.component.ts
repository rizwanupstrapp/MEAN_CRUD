...
export class DriverListComponent implements OnInit {
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.DRIVER.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.DRIVER.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.DRIVER.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.DRIVER.READ)) {
          ...
          this.api().columns().every(function () {
            ...
            switch (column) {
              ...
              case that.DRIVER_CONST.LABEL.REGISTEREDVIA:
                ...
                <option value=${appConstants.REGISTRATION_TYPE.NORMAL}>${that.DRIVER_CONST.LABEL.NORMAL}</option>
                <option value=${appConstants.REGISTRATION_TYPE.FACEBOOK}>${that.DRIVER_CONST.LABEL.FACEBOOK}</option>
                <option value=${appConstants.REGISTRATION_TYPE.GOOGLE}>${that.DRIVER_CONST.LABEL.GOOGLE}</option>
                `);
                break;

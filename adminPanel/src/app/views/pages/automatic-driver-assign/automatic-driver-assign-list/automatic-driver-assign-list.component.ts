...
@Component({
  ...
  templateUrl: './automatic-driver-assign-list.component.html',
  styleUrls: ['./automatic-driver-assign-list.component.scss']
})
export class AutomaticDriverAssignListComponent implements OnInit {
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.AUTOMATIC_DRIVER_ASSIGN.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.AUTOMATIC_DRIVER_ASSIGN.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.AUTOMATIC_DRIVER_ASSIGN.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.AUTOMATIC_DRIVER_ASSIGN.READ)) {
  ...
  ngAfterViewInit() {
    ...
    this._script.loadScripts('kt-automaticDriverAssign-list', scripts).then(function () {
      ...
      var table = $(document).find('#kt_datatable').DataTable({
        ...
        ajax: {
          url: ApiURIs.listAutomaticDriverAssign,
          type: 'POST',
          headers: {

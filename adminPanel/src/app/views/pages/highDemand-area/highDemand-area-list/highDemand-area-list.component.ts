...
@Component({
    ...
    templateUrl: './highDemand-area-list.component.html',
    styleUrls: ['./highDemand-area-list.component.scss']
})
export class HighDemandAreaListComponent implements OnInit {
    ...
    searchObj = { field: 'help_title', highDemandArea_status: '', search: '' };
    ...
    getPermission() {
        this.localStorageService.getBehaviorView().subscribe((data: any) => {
            if (data && data.length) {
                if (this.checkHasPermission(data, PERMISSIONS.HIGHDEMAND_AREA.CREATE)) {
                    this.createPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.HIGHDEMAND_AREA.EDIT)) {
                    this.editPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.HIGHDEMAND_AREA.DELETE)) {
                    this.deletePermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.HIGHDEMAND_AREA.READ)) {
    ...
    ngAfterViewInit() {
        ...
        this._script.loadScripts('kt-highDemand-area-list', scripts).then(function () {
            ...
            var table = $(document).find('#kt_datatable').DataTable({
                ...
                ajax: {
                    url: ApiURIs.listHighDemandArea,
                    type: 'POST',
                    headers: {

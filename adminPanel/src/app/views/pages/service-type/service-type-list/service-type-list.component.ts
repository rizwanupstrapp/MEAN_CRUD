...
export class ServiceTypeListComponent implements OnInit {
  ...
  searchObj = { field: "serviceType_name", serviceType_status: "", search: "" };
  ...
  constructor(
    ...
    titleService.setTitle(
      this.APPNAME + " | " + this.SERVICETYPE_CONST.MODULE_NAME
    );
  }
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.SERVICE_TYPE.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SERVICE_TYPE.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SERVICE_TYPE.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SERVICE_TYPE.READ)) {

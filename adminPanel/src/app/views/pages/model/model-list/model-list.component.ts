...
export class ModelListComponent implements OnInit {
    ...
    MODEL_CONST: any = [];
    searchObj = { field: 'model_name', model_status: '', search: '' };
    ...
    getPermission() {
        this.localStorageService.getBehaviorView().subscribe((data: any) => {
            console.log(data);
            if (data && data.length) {
                if (this.checkHasPermission(data, PERMISSIONS.MODELS.CREATE)) {

                    this.createPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.MODELS.EDIT)) {
                    this.editPermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.MODELS.DELETE)) {
                    this.deletePermission = true;
                }
                if (this.checkHasPermission(data, PERMISSIONS.MODELS.READ)) {

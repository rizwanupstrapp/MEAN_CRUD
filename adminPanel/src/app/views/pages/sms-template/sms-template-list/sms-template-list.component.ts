...
export class SmsTemplateListComponent implements OnInit {
  ...
  TEMPLATE_CONST: any = [];
  searchObj = { field: "smsTemplate_title", smsTemplate_status: "", search: "" };
  ...
  constructor(
    ...
    titleService.setTitle(
      this.APPNAME + " | " + this.TEMPLATE_CONST.SMS_MODULE_NAME
    );
  }
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.SMS_TEMPLATE.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SMS_TEMPLATE.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SMS_TEMPLATE.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SMS_TEMPLATE.READ)) {

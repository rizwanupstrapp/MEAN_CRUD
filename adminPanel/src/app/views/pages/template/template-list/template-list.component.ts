...
export class TemplateListComponent implements OnInit {
  ...
  constructor(
    ...
    titleService.setTitle(
      this.APPNAME + " | " + this.TEMPLATE_CONST.MODULE_NAME
    );
  }
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.TEMPLATE.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TEMPLATE.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TEMPLATE.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TEMPLATE.READ)) {

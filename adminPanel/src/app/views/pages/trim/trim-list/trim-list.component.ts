...
@Component({
  ...
  styleUrls: ['./trim-list.component.scss']
})
export class TrimListComponent implements OnInit {
  ...
  TRIM_CONST: any = [];
  searchObj = { field: 'trim_name', trim_status: '', search: '' };
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.TRIM.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TRIM.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TRIM.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.TRIM.READ)) {

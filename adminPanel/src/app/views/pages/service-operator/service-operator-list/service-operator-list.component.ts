...
@Component({
  ...
  templateUrl: "./service-operator-list.component.html",
  styleUrls: ["./service-operator-list.component.scss"],
})
export class ServiceOperatorListComponent implements OnInit {
  ...
  constructor(
    ...
    this.subheaderService.setTitle(
      this.translate.instant("MENU.SERVICE_OPERATOR")
    );
    titleService.setTitle(
      this.APPNAME + " | " + this.SERVICEOPERATOR_CONST.MODULE_NAME
    );
  }
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (
          this.checkHasPermission(data, PERMISSIONS.SERVICE_OPERATOR.CREATE)
        ) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SERVICE_OPERATOR.EDIT)) {
          this.editPermission = true;
        }
        if (
          this.checkHasPermission(data, PERMISSIONS.SERVICE_OPERATOR.DELETE)
        ) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.SERVICE_OPERATOR.READ)) {
      ...
      .then(function () {
        ...
        var table = $(document)
          ...
          .DataTable({
            ...
            ajax: {
              url: ApiURIs.listServiceOperator,
              type: "POST",
              headers: {

...
@Component({
  ...
  styleUrls: ['./business-owner-list.component.scss']
})
export class BusinessOwnerListComponent implements OnInit {
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.BUSINESS_OWNER.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.BUSINESS_OWNER.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.BUSINESS_OWNER.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.BUSINESS_OWNER.READ)) {
  ...
  ngAfterViewInit() {
    ...
    this._script.loadScripts('kt-business-owner-list', scripts).then(function () {
      ...
      var table = $(document).find('#kt_datatable').DataTable({
        ...
        ajax: {
          url: ApiURIs.userList + '/BUSINESS_OWNER',
          type: 'POST',
          headers: {
          ...
          this.api().columns().every(function () {
            ...
            switch (column) {
              ...
              case that.BUSINESS_CONST.LABEL.USER_PROFILE_IMAGE:
                input = $(
                  `<input type="text" class="form-control form-control-sm form-filter kt-input" data-col-index="` + this.index() + `"/>`);
                break;

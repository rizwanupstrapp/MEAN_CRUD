...
export class PromocodeListComponent implements OnInit {
  ...
  PROMOCODE_CONST: any = [];
  searchObj = { field: 'promocode_type', promocode_status: '', search: '' };
  ...
  getPermission() {
    this.localStorageService.getBehaviorView().subscribe((data: any) => {
      if (data && data.length) {
        if (this.checkHasPermission(data, PERMISSIONS.PROMOCODE.CREATE)) {
          this.createPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.PROMOCODE.EDIT)) {
          this.editPermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.PROMOCODE.DELETE)) {
          this.deletePermission = true;
        }
        if (this.checkHasPermission(data, PERMISSIONS.PROMOCODE.READ)) {
